...
%\VignetteDepends{akima, multtest}
...
%% The options
\SweaveOpts{eps=FALSE, prefix.string=pix/OCplus} 
...
You need the package \Rpackage{akima}, available from CRAN. In order to run \Rfunction{EOC}, you also need the package \Rpackage{multtest} from Bioconductor.
<<>>=
library(OCplus)
@
